...
- [Resume IPFlow bullet 措辞调整](task_resume_ipflow_bullet_措辞调整.md) — 将条 2 从独立描述改为明确承接条 1 的措辞，用'围绕上述抽奖协议'衔接
- [Resume项目经历改写](task_resume项目经历改写.md) — 改写简历两个项目经历条目，删除标签式描述，改为 achievement-focused bullet
...
- [复制 skills 到 subagent-memory 仓库](task_复制_skills_到_subagent-memory_仓库.md) — 将 agent-memory + reflex-audit + reflex-fuzz 三个 skill 复制到 ~/mem/mem/configs/skills/ 并推送
...
- [改写简历 AI 工作流项目描述](task_改写简历_ai_工作流项目描述.md) — 用真实量化数据（119次反射/97.5%成功率/320+记忆/47 Skill/134快照）改写 Resume_Web3_Fullstack_ZH.md 的 AI 增强开发工作流条目
- [改写简历 IPFlow 工作经历条目](task_改写简历_ipflow_工作经历条目.md) — 将 IPFlow 工作经历从三模块子标题格式改写为 6 条动作动词+量化指标风格的 bullet points
- [改写简历个人摘要章节](task_改写简历个人摘要章节.md) — 将 4 条 bullet 列表改写为 1 段连贯叙述段落，保留所有核心信息
- [润色简历 IPFlow 前两条 bullet](task_润色简历_ipflow_前两条_bullet.md) — 将堆砌在一行的两条 bullet 拆为三条，每条聚焦单一职责
...
- [简历 AI 项目标题改写](task_简历_ai_项目标题改写.md) — 将 AI 项目标题从'AI 增强开发工作流'改为'多智能体编排与记忆系统'，更新简介与 GitHub 链接
- [精简简历个人摘要](task_精简简历个人摘要.md) — 将 Resume_Web3_Fullstack_ZH.md 的个人摘要从 3 长句压缩为 2 句，突出链上经历/AI工作流/BFT科研三方面
...
- [补建 workflow run：简历摘要改写](task_补建_workflow_run：简历摘要改写.md) — 创建 2026-03-16-resume-summary-rewrite.md，记录两轮简历摘要改写的反射链执行情况
- [调整简历章节顺序：工作经历移至个人摘要前](task_调整简历章节顺序：工作经历移至个人摘要前.md) — 将 Resume_Web3_Fullstack_ZH.md 中的「工作经历」章节移到「个人摘要」之前，章节顺序从「目标岗位→个人摘要→核心技能→工作经历」改为「目标岗位→工作经历→个人摘要→核心技能」
...
- [重建 subagent-memory 仓库（仅配置+角色定义）](task_重建_subagent-memory_仓库（仅配置+角色定义）.md) — 清除旧 git 历史，重建仓库仅包含 configs/ 目录，排除所有 agent 记忆文件
